resource "aws_route53_record" "notifications" {
  ...
  alias {
    name                   = aws_cloudfront_distribution.notifications.domain_name
    zone_id                = aws_cloudfront_distribution.notifications.hosted_zone_id
...
resource "aws_wafv2_web_acl" "notifications" {
  ...
  visibility_config {
    ...
    metric_name                = "notifications-AclMetrics"
    sampled_requests_enabled   = true
  }
}
